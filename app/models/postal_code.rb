class PostalCode < ApplicationRecord
  ...
  def self.search(search_params)
    ...
    search_result = if country.present?
      PostalCode.where(postal_code: postal_code, country_code: country)
    else
      PostalCode.where(postal_code: postal_code)
    end

     if search_result.blank?
       postal_codes_data = ServiceCountry::Pt.search(postal_code)
       search_result = PostalCode.create(postal_codes_data)
     end
  ...
  def self.cache_address(search_params)
    ...
    if search_params[:country_code].present?
      "country/#{search_params[:country_code]}/postal_code/#{search_params[:postal_code]}"
    else
      "postal_code/#{search_params[:postal_code]}"
    end
  end
